...
URI = "bolt://neo4j:7687"  # Adjust if ports differ
USER = "neo4j"
PASSWORD = "testTEST0"
DBNAME = "neo4j"
...
def wait_for_neo4j(uri, user, password, retries=10, delay=5):
    from neo4j import GraphDatabase
    for i in range(retries):
        try:
            ...
            print(f"Neo4j not ready yet ({i+1}/{retries}): {e}")
            sleep(delay)
    raise RuntimeError("Neo4j did not become ready in time.")


def main():
    wait_for_neo4j(URI, USER, PASSWORD)
